-- Migration: Archive existing users and data for product pivot
-- Keeps users with 'scott' in their email, archives all others

-- Step 1: Create archive tables with metadata

-- Archive users table (mirrors auth.users fields we can access)
...
-- Step 2: Copy data to archive tables (only for non-Scott users)

-- Archive users (from auth.users)
...
-- Step 3: Delete archived data from active tables (in correct order for foreign keys)

-- Delete price_alerts (references card_details)
DELETE FROM public.price_alerts
WHERE user_id IN (SELECT id FROM auth.users WHERE email NOT ILIKE '%scott%');

-- Delete detections (references items)
...
-- Delete card_details (references items)
...
-- Delete shared_access
DELETE FROM public.shared_access
WHERE user_id IN (SELECT id FROM auth.users WHERE email NOT ILIKE '%scott%');

-- Delete email_tracking
...
-- Delete from auth.users (CASCADE will handle any remaining references)
DELETE FROM auth.users
WHERE email NOT ILIKE '%scott%';

-- Step 4: Enable RLS on archive tables and create admin-only policies
...
-- Create admin-only read policies for all archive tables
